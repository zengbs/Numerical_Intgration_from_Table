...
// initial check
#  ifdef GAMER_DEBUG
   if ( N <= 1 )           printf( "incorrect input parameter \"N (%d) <= 1\" !!\n", N );
   if ( Table_x == NULL )  printf( "Table_x == NULL !!\n" );
   if ( Table_y == NULL )  printf( "Table_y == NULL !!\n" );
#  endif
...
#  ifdef GAMER_DEBUG
   if ( IdxL < 0  ||  IdxL >= N-1 )
      printf( "IdxL (%d) lies outside the accepted range [%d ... %d] !!\n", IdxL, 0, N-2 );
#  endif
