...
// initial check
#  ifdef GAMER_DEBUG
   if ( Min < 0 )    printf( "incorrect input parameter \"Min (%d) < 0\" !!\n", Min );
   if ( Max <= Min ) printf( "incorrect input parameters \"Max (%d) <= Min (%d)\" !!\n", Max, Min );
#  endif
...
#  ifdef GAMER_DEBUG
   if ( Idx < Min  ||  Idx >= Max )
      printf( "incorrect output index (Idx %d, Min %d, Max%d) !!\n", Idx, Min, Max );

   if (  Array[Idx] > Key  ||  Array[Idx+1] <= Key )
      printf( "incorrect output index (Idx %d, ValueL %14.7e, ValueR %14.7e, Key %14.7e) !!\n",
